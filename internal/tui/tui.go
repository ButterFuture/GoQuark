...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case updateCheckMsg:
		...
		body := fmt.Sprintf("发现新版本：%s → %s\n", res.Current, res.Latest)
		if res.CanApply {
			body += "发现新版本，点击「立即更新」即可自动下载安装。\n"
			body += "（也可稍后按 U 手动检查；CLI: goquark update --yes）"
		} else {
			body += "有新版本，但当前环境无法自动更新（多为自编译或无对应平台资产）。\n"
			body += res.Reason + "\n\n请到发布页手动下载。\nCLI 自检: goquark update"
		}
	...
	if which == 2 {
		...
		if act == "update_available" {
			...
			if res != nil && !res.CanApply {
				// secondary already means "关闭启动时自动检查"
				if m.cfg != nil {
					m.cfg.DisableUpdateAutoCheck = true
					_ = m.cfg.Save()
				}
				m.mode = modeBrowse
				m.status = "已关闭启动时自动检查（仍可按 U 手动检查）"
				return m, nil
			}
...
func (m model) handleKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case modeDownloads:
		...
		case "p", "P": // pause all — non-blocking
			if m.dlMgr != nil {
	...
	case " ": // toggle multi-select on cursor
		...
		m.status = ""
	case "a", "A", "ctrl+a": // select all (skip ..) — case-insensitive
		m.selected = map[int]bool{}
		...
		m.status = "已全选"
	case "c", "C": // clear selection — lowercase/uppercase both OK (not Shift+A)
		m.selected = map[int]bool{}
		m.status = "已清除选择"
	case "enter", "l", "right":
		return m.doActivate()
...
func (m model) viewBrowse() string {
	...
	help := "单击选中 · 空格多选 · a全选 · A清除 · 双击/Enter打开 · 右键/m菜单 · d下载 · t下载中心 · i信息 · o退出登录 · q退出"
